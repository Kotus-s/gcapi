...
module.exports = class GCApi {
  ...
  async doRequest(requestOptions) {
    ...
    const errors = [401, 403, 404, 405, 422, 500];

    if (response && errors.includes(response.status)) {
   ...
   * @param {string} [statLabel] - Unique name for the stat, should be formatted as a slug pattern
   * @param {string} [statUnit=''] - Choose between `k`, `time` or empty
   * @param {boolean} [onHomepage=false] - Should the stat be displayed on the homepage ?
   * @return {Promise}
   */
  async createStat(statLabel, statUnit = '', onHomepage = false) {
   ...
   * @param {boolean} [shouldAppend=true] - Should append or erase the stat?
   * @return {Promise}
   */
  ...
  async getUserExperience(userId) {
    const requestHeaders = this.makeRequestHeader(
      this.makeUri({
        pathname: `/admin/users/${userId}/experience`,
      }),
    );
    const response = await this.doRequest(requestHeaders);
    return response;
   ...
   * @function
   * @public
   * @description Add a certain amount of experience to a user for a specific event
   * @async
   * @see {@link https://gmod-creator-area.gitlab.io/website/#add-users-experience|Documentation}
   * @param {integer} [userId] - The unique identifier of a user
   * @param {string} [eventName] - The unique name of the event
   * @param {integer} [amount] - Amount of exp to give to the user (can be negative)
   * @return {Promise}
   */
  async addUserExperience(userId, eventName, amount) {
    const requestHeaders = this.makeRequestHeader(
      this.makeUri({
        pathname: `/admin/users/${userId}/experience`,
      }),
  ...
  async getUserWarns(userId) {
    const requestHeaders = this.makeRequestHeader(
      this.makeUri({
        pathname: `/admin/users/${userId}/warn`,
      }),
    );
    const response = await this.doRequest(requestHeaders);
    return response;
